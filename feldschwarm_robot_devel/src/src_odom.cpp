...
namespace gazebo {
    ...
    public:
        void velocityCallback(geometry_msgs::Twist::ConstPtr ptr) {
    ...
    public:
        void asyncPublish() {
            ...
            while (ros::ok()) {
                ...
                if (std::sqrt(std::pow(this->pose.pos.x - truepose_old.x, 2) +
                              std::pow(this->pose.pos.y - truepose_old.y, 2)) > 0.001) {
                    deltarot1 = std::atan2(this->pose.pos.y - truepose_old.y, this->pose.pos.x - this->truepose_old.x) -
                                truepose_old.z;
                    deltatrans = std::sqrt(std::pow(this->pose.pos.x - truepose_old.x, 2) +
                                           std::pow(this->pose.pos.y - truepose_old.y, 2));
                } else {
                    deltarot1 = 0;
                    deltatrans = 0;
                }
                ...
                broadcaster.sendTransform(
                        tf::StampedTransform(transform, ros::Time::now(), "odom", "base_link_drifted"));
